...
<%--						<c:if test="${order.status eq 1 }">--%>
							<button type="button" class="btn btn-warning btn-sm" onclick="zhifu('${order.id}')">支付</button>
<%--							<button type="button" class="btn btn-warning btn-sm" onclick="quxiao('${order.id}')">取消</button>--%>
							<a href="${pageContext.servletContext.contextPath}/order/delete?id=${order.id}">取消</a>
<%--		${pageContext.servletContext.contextPath}/address/update?id=${address.id}--%>
<%--						</c:if>--%>
						<c:if test="${order.status eq 2 }">
